...
- 已完成：对 POST 明确拒绝状态（400、401、403、404、405、413、415、422、429）持久化清除发送意图，允许修复后显式恢复；408、5xx、网络异常等不确定结果仍禁止盲目重发。
- 已完成：恢复先按标记和作者只读核对远端通知，不因 MR HEAD 更新拒绝核对。只有确实需要新增评论时才检查 HEAD，旧 HEAD 不允许新增评论。
- 已完成：将 Webhook、CI wrapper、仓库 wrapper、TaskTool 隔离回归测试纳入运行时 CI，并在 CI 契约测试中固定这些入口。
- 回归覆盖：明确拒绝与不确定失败在重载后的不同恢复行为、HEAD 更新后核对已有评论、HEAD 更新后拒绝补发。
- 边界不变：无法确定是否送达时仅返回诊断，不自动重发；真实 GitLab 联调仍待部署验证。
