...
- `packages/platform-gitlab/src/review/project-profile-input.ts`：共享的 profile 字段表示描述、逐表示校验和合法值选择；如果实施时确认放入 `settings.ts` 更符合现有边界，可不新增文件，但后端与 Web 必须复用同一结构化校验合同。
- `packages/platform-gitlab/src/review/utf8-budget.ts`：单向 UTF-8 截断与编码字节计量；仅在提取现有实现能明显消除重复并支持确定性测试时新增。
...
- [ ] **Step 5: 运行 GREEN 与 OpenCode 运行时回归**
...
- [ ] **Step 1: 写 summary 后 HEAD 改变的 RED 测试**
...
- [ ] **Step 2: 覆盖 fallback、blocked、failure 与 commit 路径**
...
- [ ] **Step 1: 写聚合膨胀的 controller RED 测试**
...
- [ ] **Step 6: 在 API client 保留深层请求 guard**
...
- [ ] **Step 1: 写后端 collision/null RED 矩阵**
...
- [ ] **Step 2: 运行根测试、三处 typecheck 与 Web build**
...
| Task 1 | specialist 显式空资源快照 | 待实施 | 真实 prompt 后可复用且无全局资源注入 |
| Task 2 | 每次 MR POST 紧邻 HEAD 校验 | 待实施 | HEAD 改变后零后续 POST，错误原样返回 |
| Task 3 | claim 前完整发布计划预算 | 待实施 | aggregate/render/form 超限均零 claim、零网络、HTTP 413 |
| Task 4 | profile 逐表示无损校验 | 待实施 | collision/null 全覆盖，无关编辑不丢原值，复杂度测试确定性 |
| Task 5 | 全量验证、最终复审、文档与推送 | 待实施 | open Critical/Important 为 0，远端 SHA 等于本地 HEAD |
