...
Webhooks 页面支持外部服务通过 JSON 请求触发绑定项目下的 agent 运行。每个来源都有系统生成的 URL 和 secret，完整 URL 只会在创建来源或刷新 secret 后展示一次。

内置 preset 提供 Generic JSON、Uptime Kuma 告警、GitLab 合并请求审查的快速起点。preset 只会预填字段映射、提示词、示例请求和防护配置，创建后的 source 仍可自由编辑。
